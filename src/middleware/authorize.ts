...
import { rolePermissions, permissions } from '../infrastructure/db/schema/index.js';
...
async function loadPermissions(role: string): Promise<Set<string>> {
  ...
  const rows = await db
    ...
    .innerJoin(
      ...
      eq(rolePermissions.roleId, (await import('../infrastructure/db/schema/index.js')).roles.id),
    )
    .where(
      eq((await import('../infrastructure/db/schema/index.js')).roles.name, role),
    );
