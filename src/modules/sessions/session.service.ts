...
export async function createSession(params: {
      ...
      const [session] = await db
            ...
            .values({
                  ...
                  expiresAt: new Date(
                        Date.now() + env.JWT_REFRESH_TOKEN_TTL * 1000,
                  ),
            })
...
export async function revokeSession(
      ...
      // Cache revocation in Redis for real-time session invalidation
      try {
            const redis = getRedis()
            const env = getEnv()
            await redis.set(
                  RedisKeys.revokedSession(sessionId),
                  '1',
                  'EX',
                  env.JWT_ACCESS_TOKEN_TTL,
            )
      } catch {
            // Best-effort: Redis failure must not block session revocation
      }
}
...
export async function revokeAllUserSessions(
      ...
      const activeSessions = await db
            ...
            .from(sessions)
            .where(
                  and(eq(sessions.userId, userId), eq(sessions.revoked, false)),
            )
...
export async function isSessionValid(sessionId: string): Promise<boolean> {
      ...
      const idleThreshold = new Date(
            Date.now() - SESSION_IDLE_TIMEOUT_SEC * 1000,
      )
