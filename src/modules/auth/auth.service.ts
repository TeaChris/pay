...
import {
      createRefreshToken,
      rotateRefreshToken,
} from '../tokens/refresh.service.js'
import {
      signAccessToken,
...
export async function login(params: LoginParams) {
      ...
      if (!user) {
            await verifyPassword(getDummyHash(), params.password)
...
export async function refresh(params: RefreshParams) {
      const { newToken, sessionId, userId } = await rotateRefreshToken(
            params.rawRefreshToken,
      )

      const profile = await getUserProfile(userId)

      // Read MFA verification status from the session record, not from user's mfaEnabled flag
      const db = getDb()
      const [sessionRecord] = await db
            .select({ mfaVerified: sessions.mfaVerified })
            .from(sessions)
      ...
      const accessToken = await signAccessToken({
            ...
            sessionId,
            role: profile.role,
            mfaVerified: sessionRecord?.mfaVerified ?? false,
      })
